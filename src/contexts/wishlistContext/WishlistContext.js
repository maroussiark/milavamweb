...
const WishlistContextProvider = ({ children }) => {
  ...
  const addProductToWishlist = async (annonce) => {
    setDisableWish(true);
    try {
      const response = await api.post("/user/annonce/favoris/ajouter/"+annonce.id);
      console.log(response);
      if (response.status === 200 || response.status === 201) {
        updateInCartOrInWish(annonce.id, "inWish", true);
      }
